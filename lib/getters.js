...
module.exports = function(Queue) {
  ...
  */
  Queue.prototype.count = function() {
    return Promise.all([
      this.getJobCountByTypes('wait', 'paused', 'delayed'),
      this.client.zcard(this.keys.prioritized)
    ]).then(([count, prioritizedCount]) => count + prioritizedCount);
  };
  ...
  Queue.prototype.getJobCounts = function() {
    ...
    // 'waiting' aliases to the plain 'wait' list, but prioritized jobs live
    // in their own zset (see addJobWithPriority.lua) and are also waiting
    // to be processed, so fold their count in here too.
    const waitingIndex = types.indexOf('waiting');
    if (waitingIndex !== -1) {
      multi.zcard(this.keys.prioritized);
    }

    return multi.exec().then(res => {
      const counts = {};
      types.forEach((type, index) => {
        counts[type] = res[index][1] || 0;
      });
      if (waitingIndex !== -1) {
        counts.waiting += res[types.length][1] || 0;
      }
      return counts;
    });
  };
  ...
  Queue.prototype.getWaitingCount = function() {
    return Promise.all([
      this.getJobCountByTypes('wait', 'paused'),
      this.client.zcard(this.keys.prioritized)
    ]).then(([count, prioritizedCount]) => count + prioritizedCount);
  };
  ...
  // NOTE: prioritized jobs are fetched in full (ignoring start/end), then
  // prepended to the wait/paused results, since they're processed first.
  // Arbitrary pagination windows spanning the prioritized/wait boundary
  // are not handled precisely.
  Queue.prototype.getWaiting = function(start, end, opts) {
    return Promise.all([
      this.client.zrange(this.keys.prioritized, 0, -1),
      this.getJobs(['wait', 'paused'], start, end, true, opts)
    ]).then(([prioritizedJobIds, waitingJobs]) => {
      return Promise.all(
        prioritizedJobIds.map(jobId => this.getJobFromId(jobId, opts))
      ).then(prioritizedJobs => prioritizedJobs.concat(waitingJobs));
    });
  };
