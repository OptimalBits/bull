...
/*
  interface QueueOptions {
    ...
    settings?: QueueSettings {
      ...
      initChildProcess?: boolean = false
    }
  }
...
Queue.prototype.process = function(name, concurrency, handler) {
  ...
  return Promise.all([this._initProcess(), this._initChildProcess(handler)]).then(() => {
...
Queue.prototype.setHandler = function(name, handler) {
  ...
  if (typeof handler === 'string') {
    ...
    const { isSharedChildPool } = this.settings;
...
/**
 * Initializes forked process if initChildProcess is true otherwise just
 * initializes
 */
Queue.prototype._initChildProcess = function (processFile) {
  if(typeof processFile === 'string') {
    ...
    const { initChildProcess } = this.settings;
    if (initChildProcess === true &&
