...
class Queue extends EventEmitter {
          ...
          .then(() => {
            ...
            if (['active', 'waiting', 'stalled'].indexOf(_eventName) !== -1) {
              return (this.registeredEvents[
                _eventName
              ] = this.eclient.psubscribe(channel + '*'));
            } else {
              return (this.registeredEvents[
                _eventName
              ] = this.eclient.subscribe(channel));
            }
          })
          .then(() => {
  ...
  setHandler(name, handler) {
    ...
    if (typeof handler === 'string') {
      ...
      this.childPool =
        this.childPool || new (require('./process/child-pool'))();
  ...
  */
  ...
  _processJobOnNextTick(processJobs, index, resolve, job) {
    if (!this.closing) {
