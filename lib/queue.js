...
Queue.prototype.updateDelayTimer = function(delayedTimestamp){
  ...
  if(delayedTimestamp < _this.delayedTimestamp){
    ...
    this.delayTimer = setTimeout(function(){
      updateDelaySet(_this).then(function(nextTimestamp){
        _this.delayedTimestamp = Number.MAX_VALUE;
        if(nextTimestamp){
          var delayed = _this.toKey('delayed');
          _this.client.publish(delayed, nextTimestamp);
        }
      })
    }, nextDelayedJob);
  }
...
*/
var updateDelaySet = function(queue){
  var script = [
    ...
    '  return redis.call("ZRANGE", KEYS[1], 0, 0, "WITHSCORES")[2]',
    ' end',
    'end'].join('\n');
...
Queue.prototype.processJob = function(job){
  ...
  if(job.delay){
    ...
    if(jobDelayedTimestamp > Date.now()){
      _this.client.publish(_this.toKey('delayed'), jobDelayedTimestamp);
