...
const Queue = function Queue(name, url, opts) {
  ...
  const lazyClient = redisClientGetter(this, opts, (type, client) => {
    ...
    if (type === 'client') {
      function initScripts(instance) {
        return commands(client).then(
            () => {
              debuglog(name + ' queue ready');
            },
            err => {
              instance.emit('error', new Error('Error initializing Lua scripts'));
              instance._initializing = initScripts(instance);
            }
        );
      }
      this._initializing = initScripts(this);
