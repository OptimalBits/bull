...
var Queue = function Queue(name, redisPort, redisHost, options){
  if(!(this instanceof Queue)){
    return new Queue(name, redisPort, redisHost, options);
  }
  ...
  if(_.isObject(redisPort)) {
    options = redisPort;
  } else if (_.isObject(redisPort)) {
    options = redisPort;
  } else if (_.isObject(redisHost)) {
    options = redisHost;
  }
  ...
  if(_.isString(redisPort)) {
    try {
      ...
      options.redis.port = redisUrl.port;
      options.redis.host = redisUrl.hostname;
      if (redisUrl.auth) {
        options.redis.password = redisUrl.auth.split(':')[1];
      }
    } catch (e) {
      throw new Error(e.message);
    }
  } else {
    if(_.isNumber(redisPort)) options.redis.port = redisPort;
    if(_.isString(redisHost)) options.redis.host = redisHost;
  }

  options.redis.port = options.redis.port || 6379;
  options.redis.host = options.redis.host || '127.0.0.1';
  options.redis.db = options.redis.db || options.redis.DB;

  function createClient() {
    var client;
    if(_.isFunction(options.redis.createClient)){
      client = options.redis.createClient();
    }else{

      client = redis.createClient(options.redis.port, options.redis.host, _.extend({}, options.redis, { keyPrefix: false }));
    }
    return client;
  }
  ...
  this.name = name;
  this.keyPrefix = options.prefix || 'bull';
  ...
  if (options.clients) {
    this.clients.push.apply(this.clients, options.clients);
  }
  this.redlock = {
  ...
  _.extend(this.redlock, options.redlock || {});
