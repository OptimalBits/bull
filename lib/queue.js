...
//
// TODO: an empty lists does not exist, therefore we need another mechanism
// to tell the queue that it is paused (for example a hash meta with )
...
Queue.prototype.processStalledJobs = function(){
  ...
  return this.client.lrangeAsync(this.toKey('active'), 0, -1).then(function(jobs){
    var tasks = jobs.map(function(jobId){
      return function() {
        Job.fromId(_this, jobId).then(_this.processStalledJob.bind(_this));
      }
    });
