...
var Queue = function Queue(name, redisPort, redisHost, redisOptions){
  ...
  // Shared redis client may be defined here too
  if(redisOptions && redisOptions.sharedClient)
    this.client = redisOptions.sharedClient;

  // If still not redis client, create as usual
  if(!this.client)
