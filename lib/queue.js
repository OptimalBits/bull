  ...
  if (typeof handler === 'string') {
    ...
    if (handler.length > 1) {
      ...
      this.handlers[name] = function() {
        try {
          const maybeThenable = handler.apply(null, arguments)
          if(maybeThenable.then) {
...
Queue.prototype.processJob = function(job, notFetch = false) {
  ...
  if (!handler) {
    ...
    if (timeoutMs) {
      let cancelHandler
      if(jobPromise.cancel) {
        cancelHandler = jobPromise.cancel.bind(jobPromise)
      }
      jobPromise = pTimeout(jobPromise, timeoutMs);
      jobPromise.cancel = cancelHandler
    }
