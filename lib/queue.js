...
Queue.prototype.init = function(name){
  var _this = this;
...
Queue.prototype._setupQueueEventListeners = function(){
  ...
  */
  ...
  this.eclient.on('pmessage', function(channel, pattern, message){
    ...
    // TODO: refactor. Idea, why not emit the local events directly
    // instead of having this token mechanism? seems unnecessary.
    switch(key){
      case activeKey:
      ...
      case progressKey:
        ...
        if(_this.token === token){
          _this.emit('progress', jobAndProgress[0], jobAndProgress[1]);
        }
...
/**
  ...
  /*
  queue.work('export', function(job, input){
