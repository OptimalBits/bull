...
var Queue = function Queue(name, redisPort, redisHost, redisOptions){
  ...
  this.paused = false;
  this.delayedTimestamp = Math.MAX_INT;
  this.delayTimer;
...
Queue.prototype.updateDelayTimer = function(){
  ...
  if(this.delayedTimestamp < Math.MAX_INT){
    this.delayTimer = setTimeout(function(){
      //
      // Pop a job from from delayed queue, check if timestamp + delay is larger than now
      // if so, stop listening to the queue and start processing this job.
      // if not, put it again into the sorted set. This should be done using a
      // lua script.
      //

    }, this.delayedTimestamp);
  }
}
...
Queue.prototype.getNextJob = function(){
  ...
  //
  // Check if there are any delayed jobs that need to be processed
  //

  return this.moveJob('wait', 'active').then(getJobFromId);

  //
  // Check if no other job is being processed right now, if so we need to move it to the delay set
  // until we finalize the current job.
  //
}
