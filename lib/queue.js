...
Queue.prototype.empty = function() {
  ...
  multi.zrange(queueKeys.prioritized, 0, -1);
  ...
  return multi.exec().then(res => {
    let [waiting, paused, prioritized, limited] = res;
...
Queue.prototype.getNextJob = async function() {
  ...
  if (this.drained) {
    //
    // Waiting for new jobs to arrive. The marker is only a wake-up signal:
    // producers push a token onto it whenever a job becomes available in
    // 'wait' or 'prioritized'. Whether we wake up because of a marker or
    // simply time out, we always attempt a real fetch afterwards: a timeout
    // fetch is what lets us pick up a pre-existing backlog (e.g. legacy jobs
    // present before this worker started) that nobody pushed a marker for.
    // This can't cause duplicate 'drained' events since that emission is
    // edge-triggered.
    //
    try {
      this.bclient.blocked = true;
      await this.bclient.brpop(this.keys.marker, this.settings.drainDelay);
