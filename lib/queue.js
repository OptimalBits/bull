...
Queue.prototype.start = function(concurrency) {
  ...
  return this.run(concurrency).catch(function(err) {
    console.log('ERROR RUNNING QUEUE');
  ...
  process.nextTick(function() {
    if (!_this.closing) {
      (_this.paused || Promise.resolve())
        ...
        .catch(function(err) {
          console.log('THIS ERROR IS UNRECOVERABLE');
          _this.emit('error', err, 'Error processing job');
        });
    } else {
...
Queue.prototype.getNextJob = function() {
  ...
  if (this.drained) {
    ...
    //
    console.log('BLOCKING WAITING');
    return this.bclient
      .brpoplpush(this.keys.wait, this.keys.active, _this.settings.drainDelay)
      .then(
        function(jobId) {
          console.log('GOT JOB; MAYBE', jobId);
          if (jobId) {
            return _this.moveToActive(jobId);
          }
        },
        function(err) {
          console.log('ARE WE SWALLING AN ERROR?', err);
          // Swallow error
          if (err.message !== 'Connection is closed.') {
