...
var Queue = function Queue(name, redisConfig){
  ...
  this.name = name;
  this.prefix = redisConfig.prefix || 'bull';

  function createClient() {
    var client;
    if(_.isFunction(redisConfig.createClient)){
  ...
  this.client.selectAsync(redisConfig.db).then(function(){
    return _this.bclient.selectAsync(redisConfig.db);
  }).then(function(){
    return _this.eclient.selectAsync(redisConfig.db);
  }).then(function(){
...
Queue.prototype.toKey = function(queueType){
  return [this.prefix, this.name, queueType].join(':');
};
