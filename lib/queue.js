...
var Queue = function Queue(name, redisPort, redisHost, redisOptions){
  ...
  if(_.isObject(redisPort)){
    ...
    this._redisPort = redisOpts.port || this._redisPort;
    this._redisHost = redisOpts.host || this._redisHost;
    this._redisOptions = redisOpts.opts || this._redisOptions;
    this._redisDB = redisOpts.DB || this._redisDB;

    // Shared redis client may be defined in options
    if (opts.sharedClient)
      this.client = opts.sharedClient || null;
  }
  else {
  ...
  // Redis client may be already defined:
  // - from redisPort as options (see above)
  // - or defined in redis options
  this.client = this.client || (this._redisOptions && this._redisOptions.sharedClient);

  // If still no redis client, create it as usual
  if(!this.client)
    this.client = redis.createClient(this._redisPort, this._redisHost, this._redisOptions);
  ...
  this.client.select(this._redisDB, function(err){
    _this.emit('ready');
  });
}
...
/**
  Initialize bclient
*/
Queue.prototype._initBClient = function(){
  var _this = this;

  this.bclient = redis.createClient(this._redisPort, this._redisHost, this._redisOptions);
  ...
  this.bclient.select(_this._redisDB, function(err) {
    _this.emit('ready_process');
  });
}
