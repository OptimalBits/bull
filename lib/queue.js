...
var Queue = function Queue(name, url, opts) {
  ...
  var lazyClient = redisClientGetter(this, opts, function(type, client) {
    ...
    if (type === 'client') {
      ...
      _this._initializing.catch(function(/*err*/) {});
    }
  });
...
Queue.prototype._setupQueueEventListeners = function() {
  ...
  */
  ...
  this.eclient.on('message', function(channel, message) {
    ...
    switch (key) {
      case progressKey:
        var commaPos = message.indexOf(',');
        var jobId = message.substring(0, commaPos);
        var progress = message.substring(commaPos + 1);
        _this.emit('global:progress', jobId, JSON.parse(progress));
        break;
      case delayedKey:
...
Queue.prototype.add = function(name, data, opts) {
  ...
  if (opts.repeat) {
    ...
    return this.isReady().then(function() {
      return _this.nextRepeatableJob(name, data, opts, true);
    });
  } else {
 ...
 * except that it will return the stalled jobs instead of moving them to active.
...
Queue.prototype.getStalledJobs = function() {
  ...
  return scripts
    ...
    .then(function(res) {
      ...
      var jobs = failed.concat(stalled).map(function(jobId) {
        return _this.getJobFromId(jobId);
      });
      return Promise.all(jobs);
    })
    .catch(function(err) {
      var error = new Error('Failed to get stalled jobs, ' + err.message);
      _this.emit('error', err, error.message);
...
Queue.prototype.processJob = function(job) {
  ...
  lockExtender();
  var handler = _this.handlers[job.name] || _this.handlers['*'];
...
Queue.prototype.moveToActive = function(jobId) {
  var _this = this;
  return scripts.moveToActive(_this, jobId).then(function(res) {
    var jobData = res[0],
      jobId = res[1];
