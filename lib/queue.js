...
Queue.prototype.on = function(){
  var args = Array.prototype.slice.call(arguments);
  args[0] = args[2] ? args[0] + '@' + this.name : args[0];
...
Queue.prototype.processJob = function(job){
  ...
  var lockRenewer = function(){
    try {
      ...
      console.error('Caught renew lock problem ' + err);
    }
  };
  ...
  function handleFailed(err){
    _this.processing--;
    var error = err.message || err.cause || err; //Handle explicit rejection
  ...
  return lockRenewer().then(function(locked){
    if(locked){
      ...
      _this.distEmit('active', job.toJSON());
