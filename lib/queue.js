...
const { ChildPool } = require('./process/child-pool');
...
const Queue = function Queue(name, url, opts) {
  ...
  if(this.settings.childPool !== null && this.settings.childPool instanceof ChildPool) {
   this.childPool = this.settings.childPool;
  }
...
Queue.prototype.close = async function(doNotWaitJobs) {
  let isReady = true;
  if (this.closing) {
    return this.closing;
  }
