...
Job.prototype.retry = function() {
  return this.queue.isReady().then(async function() {
    this.failedReason = null;
    this.finishedOn = null;
    this.processedOn = null;
    this.retriedOn = Date.now();
    const queuePaused = await this.queue.isPaused();

    return scripts.reprocessJob(this, { state: 'failed', queuePaused }).then(result => {
      if (result === 1) {
        return;
      } else if (result === 0) {
