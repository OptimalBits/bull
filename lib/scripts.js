...
const scripts = {
  ...
  pause(queue, pause) {
    ...
    const keys = _.map(
      [src, dst, 'meta-paused', pause ? 'paused' : 'resumed', 'meta', 'marker'],
      name => {
  ...
  moveToActive(queue, jobId) {
    ...
    keys[8] = queueKeys.prioritized;
    keys[9] = queueKeys['meta-paused'];
  ...
  */
  ...
  retryJobArgs(job, ignoreLock) {
    ...
    const keys = _.map(
      ['active', 'wait', jobId, 'meta-paused', 'paused', 'stalled', 'prioritized', 'marker'],
      name => {
        return queue.toKey(name);
      }
    );
