...
var scripts = {
  ...
  moveToFinished: function(job, val, propVal, shouldRemove, target, ignoreLock){
    ...
    return job.queue.client.moveToFinished(args).then(function(result){
      if(result < 0){
        throw scripts.finishedErrors(result, job.id);
      }
    });
  },

  finishedErrors: function(code, jobId){
    switch(code){
      case -1: return new Error('Missing key for job ' + jobId);
      case -2: return new Error('Missing lock for job ' + jobId);
    }
  },
