...
const scripts = {
  isJobInList(client, listKey, jobId) {
    return client.isJobInList([listKey, jobId]).then(result => result === 1);
  },
  ...
  updateProgress(job, progress) {
    const { queue } = job;
    const keys = [job.id, 'progress'].map(name => queue.toKey(name));
  ...
  moveToCompleted(job, returnvalue, removeOnComplete, ignoreLock, notFetch) {
  ...
  isFinished(job) {
    const keys = _.map(['completed', 'failed'], key => job.queue.toKey(key));
  ...
  moveToDelayedArgs(queue, jobId, timestamp, ignoreLock) {
    ...
    const keys = _.map(['active', 'delayed', jobId], name => queue.toKey(name));
    return keys.concat([
  ...
  moveToDelayed(queue, jobId, timestamp, ignoreLock) {
    ...
    return queue.client.moveToDelayed(args).then(result => {
      switch (result) {
        case -1:
          throw new Error(
            `Missing Job ${jobId} when trying to move from active to delayed`
          );
        case -2:
          throw new Error(
            `Job ${jobId} was locked when trying to move from active to delayed`
          );
      }
    });
  },
  ...
  */
  ...
  retryJobArgs(job, ignoreLock) {
    ...
    const keys = _.map(['active', 'wait', jobId], name => queue.toKey(name));
