...
var scripts = {
  ...
  moveToActive: function(queue){
    ...
    return runScript(queue.client, 'moveToActive', lua, keys, args).then(function(result){
      if(result){
  ...
  moveToDelayed: function(queue, jobId, context){
    var lua = loadScriptIfNeeded(queue.client, 'moveToDelayed');
    ...
    return runScript(queue.client, 'moveToDelayed', lua, keys, [JSON.stringify(context), jobId]).then(function(result){
      if(result === -1){
