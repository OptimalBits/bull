...
var scripts = {
  ...
  moveToDelayedArgs: function(queue, jobId, timestamp){
    //
    ...
    return keys.concat([JSON.stringify(timestamp), jobId, queue.token]);
  },

  moveToDelayed: function(queue, jobId, timestamp){
    var args = scripts.moveToDelayedArgs(queue, jobId, timestamp);
